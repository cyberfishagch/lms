...
# Fields stripped from every doc before re-inserting as a clone. Includes
# both metadata (name/timestamps/owner/docstatus) and parent-child linkage
# (parent/parenttype/parentfield) so child-row dicts can be reused safely.
_CLONE_META_FIELDS = frozenset({
	"name",
	"creation",
	"modified",
	"owner",
...
def _strip_meta(row: dict) -> dict:
	"""Drop Frappe-managed metadata from a dict so it can be re-inserted."""
	return {k: v for k, v in row.items() if k not in _CLONE_META_FIELDS}


def _clone_quiz(source_quiz_name: str) -> str:
	"""Deep-copy an LMS Quiz including its questions child table.

	Returns the new quiz's doc name. Quiz questions are themselves links to
	separate LMS Question docs; we copy the child rows pointing at the same
	questions (admins typically want to remix the lineup, not duplicate every
	question). Child rows have their metadata stripped so Frappe assigns
	fresh names + parent linkage on insert.
	"""
	source = frappe.get_doc("LMS Quiz", source_quiz_name)
	source_dict = source.as_dict()

	# Top-level scalars + the questions child rows (stripped of metadata).
	data = {"doctype": "LMS Quiz"}
	for fieldname, value in source_dict.items():
		if fieldname in _CLONE_META_FIELDS:
			continue
		if fieldname == "questions" and isinstance(value, list):
			data["questions"] = [_strip_meta(row) for row in value]
			continue
		data[fieldname] = value
	data["title"] = f"{source.title} (copy)"

	new_quiz = frappe.get_doc(data)
	new_quiz.insert(ignore_permissions=True)
	return new_quiz.name


@frappe.whitelist()
def clone_course(source_name: str, also_duplicate_quizzes: int = 0) -> dict:
	"""Deep-clone an LMS Course as a draft.

	Creates a new course with title "<original> (copy)", `published = 0`, every
	chapter and lesson duplicated under the new course, and the cloning admin
	appended to the instructor list. Learner data (enrollments, progress,
	submissions, certificates) is NOT carried over — the clone starts empty.

	The whole operation runs in the request's Frappe transaction; any failure
	rolls everything back so we never leave half-cloned content behind.

	Quiz references on lessons stay pointed at the source's `LMS Quiz` docs by
	default, since most admins want to reuse the same quiz across variants.
	Pass `also_duplicate_quizzes=1` to deep-copy referenced quizzes too.

	IMPORTANT: this endpoint does NOT manually create `Chapter Reference` or
	`Lesson Reference` child rows. Those are regenerated by the existing
	`on_update` hooks on `CourseChapter` / `CourseLesson` after each insert.
	Hand-building those tables here caused issue #92 (duplicate ref rows on
	subsequent saves) and we explicitly avoid that path.

	Args:
		source_name: doc name of the LMS Course to clone.
		also_duplicate_quizzes: when truthy, every lesson's `quiz_id` is
			rewritten to point at a freshly cloned LMS Quiz. Defaults to 0.

	Returns:
		dict with the new course name, title, and per-doctype counts (useful
		for the admin UI's success toast).

	Raises:
		frappe.PermissionError: caller lacks `can_modify_course` on source.
		frappe.DoesNotExistError: source course not found.
	"""
	if not can_modify_course(source_name):
		frappe.throw(
			_("You do not have permission to clone this course."),
			frappe.PermissionError,
		)
	...
	also_duplicate_quizzes = cint(also_duplicate_quizzes)
	source = frappe.get_doc("LMS Course", source_name)
	source_dict = source.as_dict()

	# Fields we never copy onto the new course doc. Includes learner-data
	# aggregates (enrollments/lessons counts, rating), publishing state, and
	# the `chapters` / `related_courses` child tables — `chapters` is
	# regenerated by the Chapter on_update hook below; `related_courses`
	# would create dangling references to the source's siblings.
	skip_course_fields = _CLONE_META_FIELDS | {
		"enrollments",
		"lessons",
		"rating",
		"notification_sent",
		...
		"chapters",
		"related_courses",
		"instructors",  # rebuilt below to inject the calling user
	}
	...
	# they weren't already an instructor).
	instructor_users = []
	seen = set()
	for row in source.instructors or []:
		if row.instructor and row.instructor not in seen:
			instructor_users.append(row.instructor)
			seen.add(row.instructor)
	if frappe.session.user not in seen and frappe.session.user != "Guest":
		instructor_users.append(frappe.session.user)
	new_course_data["instructors"] = [{"instructor": u} for u in instructor_users]

	try:
		new_course = frappe.get_doc(new_course_data)
		...
		# Source chapters in display order. Insert order determines new idx.
		source_chapters = frappe.get_all(
			"Course Chapter",
			filters={"course": source_name},
			fields=["name"],
			order_by="idx asc",
		)
		...
		skip_chapter_fields = _CLONE_META_FIELDS | {
			"course",
			"course_title",
			"lessons",  # regenerated by Course Lesson on_update hook
		}
		...
		for sc in source_chapters:
			...
			source_lessons = frappe.get_all(
				"Course Lesson",
				filters={"chapter": source_chapter.name, "course": source_name},
				fields=["name"],
				order_by="idx asc",
			)
			...
			for sl in source_lessons:
				...
				quiz_id = new_lesson_data.get("quiz_id")
				if quiz_id and also_duplicate_quizzes:
					if quiz_id not in quiz_map:
						quiz_map[quiz_id] = _clone_quiz(quiz_id)
